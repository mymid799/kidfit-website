...
const UserDatabaseManager = () => {
    ...
    const fetchUsers = useCallback(async () => {
        ...
        try {
            ...
            if (!res.ok) throw new Error('Không thể kết nối tới API! (Bạn có quyền Admin không?)');
            const data = await res.json();

            // Map backend snake_case and nested profile to frontend camelCase
            const mappedUsers = (data.users || []).map((u: any) => ({
                id: u.id,
                parentName: u.parentProfile?.parent_name || u.username || 'N/A',
                email: u.email,
                phone: u.parentProfile?.phone || 'N/A',
                role: u.role,
