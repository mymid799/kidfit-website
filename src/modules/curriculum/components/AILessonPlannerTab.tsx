...
const AILessonPlannerTab = () => {
    ...
    const handleSaveLesson = async () => {
        ...
        try {
            ...
            const res = await fetch(`${API}/teacher/materials/lessons`, {
                method: 'POST',
                headers: {
                ...
                body: JSON.stringify({
                    title: result.title,
                    description: result.story.substring(0, 200) + '...',
                    content_type: 'STEAM',
                    grade_block: formData.ageGroup === '3-4 Tuổi' ? 'MẦM' : formData.ageGroup === '4-5 Tuổi' ? 'CHỒI' : 'LÁ',
                    tags: [result.topic_tag, 'AI_GENT'],
                    // Lưu trữ thêm metadata nếu cần
                })
            });

            const data = await res.json();
            if (data.success) {
                alert('Đã lưu giáo trình vào hồ sơ cá nhân thành công!');
                // Reset hoặc chuyển sang tab hồ sơ nếu cần
                reset();
            } else {
                alert(data.error || 'Lỗi khi lưu giáo trình');
            }
        } catch (error) {
        ...
        <div className="max-w-7xl mx-auto space-y-10 pb-12 animate-in fade-in duration-500">
            ...
            {!result && (
                <section>
                    <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-6">
                        <div>
                            <h2 className="text-3xl font-extrabold text-slate-800 uppercase tracking-tight">AI Lesson Planner</h2>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                        <div className="bg-green-50/80 border border-green-100 p-6 rounded-[32px] relative overflow-hidden group hover:shadow-md transition-all cursor-pointer">
                            <div className="relative z-10 flex flex-col h-full justify-between gap-4">
                                <span className="material-symbols-outlined text-green-600 text-[28px]" style={{ fontVariationSettings: "'FILL' 1" }}>event</span>
                                <div>
                                    <h3 className="font-bold text-sm text-green-800 mb-1">Bài giảng hôm nay</h3>
                                    <p className="text-[32px] leading-none font-black text-green-600">03</p>
                                </div>
                            </div>
                            <div className="absolute right-[-10px] bottom-[-10px] opacity-10 group-hover:scale-110 transition-transform duration-500">
                                <span className="material-symbols-outlined text-8xl text-green-600">event_available</span>
                            </div>
                        </div>
                        
                        <div className="bg-blue-50/80 border border-blue-100 p-6 rounded-[32px] relative overflow-hidden group hover:shadow-md transition-all cursor-pointer">
                            <div className="relative z-10 flex flex-col h-full justify-between gap-4">
                                <span className="material-symbols-outlined text-blue-600 text-[28px]" style={{ fontVariationSettings: "'FILL' 1" }}>calendar_view_week</span>
                                <div>
                                    <h3 className="font-bold text-sm text-blue-800 mb-1">Tuần này</h3>
                                    <p className="text-[32px] leading-none font-black text-blue-600">15</p>
                                </div>
                            </div>
                            <div className="absolute right-[-10px] bottom-[-10px] opacity-10 group-hover:scale-110 transition-transform duration-500">
                                <span className="material-symbols-outlined text-8xl text-blue-600">date_range</span>
                            </div>
                        </div>
                        
                        <div className="bg-amber-50/80 border border-amber-100 p-6 rounded-[32px] relative overflow-hidden group hover:shadow-md transition-all cursor-pointer">
                            <div className="relative z-10 flex flex-col h-full justify-between gap-4">
                                <span className="material-symbols-outlined text-amber-600 text-[28px]" style={{ fontVariationSettings: "'FILL' 1" }}>library_books</span>
                                <div>
                                    <h3 className="font-bold text-sm text-amber-800 mb-1">Tổng bài giảng</h3>
                                    <p className="text-[32px] leading-none font-black text-amber-600">124</p>
                                </div>
                            </div>
                            <div className="absolute right-[-10px] bottom-[-10px] opacity-10 group-hover:scale-110 transition-transform duration-500">
                                <span className="material-symbols-outlined text-8xl text-amber-600">menu_book</span>
                            </div>
                        </div>
                        
                        <div className="bg-red-50/80 border border-red-100 p-6 rounded-[32px] relative overflow-hidden group hover:shadow-md transition-all cursor-pointer">
                            <div className="relative z-10 flex flex-col h-full justify-between gap-4">
                                <span className="material-symbols-outlined text-red-500 text-[28px]" style={{ fontVariationSettings: "'FILL' 1" }}>waves</span>
                                <div>
                                    <h3 className="font-bold text-sm text-red-800 mb-1">Chủ đề hiện tại</h3>
                                    <p className="text-[17px] leading-tight font-black text-red-500 mt-0.5 max-w-[130px] pr-2 truncate">Khám phá Đại dương</p>
                                </div>
                            </div>
                            <div className="absolute right-[-10px] bottom-[-10px] opacity-10 group-hover:scale-110 transition-transform duration-500">
                                <span className="material-symbols-outlined text-8xl text-red-500">water</span>
                            </div>
                        </div>
                    </div>
                </section>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                    ...
                    <section className="bg-white p-8 rounded-[40px] border border-slate-100 shadow-sm relative overflow-hidden">
                        {isGenerating && (
                            <div className="absolute inset-0 z-10 bg-white/80 backdrop-blur-sm flex flex-col items-center justify-center text-center p-6">
                                ...
                                <p className="font-black text-slate-800 uppercase tracking-widest text-xs">AI đang soạn bài giảng...</p>
                        ...
                        <div className="flex items-center gap-3 mb-8">
                            ...
                            <h2 className="text-xl font-black text-slate-800 uppercase tracking-tight">Thiết kế nhanh</h2>
                        </div>
                        ...
                        <div className="space-y-6">
                            <div>
                                ...
                                <input 
                                    className="w-full bg-slate-50 border-none rounded-2xl px-5 py-4 text-sm font-bold focus:ring-4 focus:ring-primary/20 transition-all" 
                            ...
                            <div className="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <select 
                                        className="w-full bg-slate-50 border-none rounded-2xl px-5 py-4 text-sm font-bold focus:ring-4 focus:ring-primary/20"
                                        name="ageGroup"
                            ...
                            <div>
                                ...
                                <div className="flex items-center gap-4 bg-slate-50 rounded-2xl px-4 py-2">
                                    <button onClick={() => adjustActivities(-1)} className="size-10 rounded-xl bg-white shadow-sm flex items-center justify-center font-black text-slate-600 hover:bg-primary hover:text-white transition-all">-</button>
                                    <span className="flex-1 text-center font-black text-lg text-slate-800">{formData.activityCount}</span>
                                    <button onClick={() => adjustActivities(1)} className="size-10 rounded-xl bg-white shadow-sm flex items-center justify-center font-black text-slate-600 hover:bg-primary hover:text-white transition-all">+</button>
                                </div>
                            </div>
                ...
                <div className="lg:col-span-8">
                    <section className="bg-white rounded-[40px] border border-slate-100 shadow-sm min-h-[600px] flex flex-col overflow-hidden">
                        {!result ? (
                            ...
                            <>
                                <div className="p-10 border-b border-slate-50 flex flex-col md:flex-row md:items-center justify-between gap-6 bg-slate-50/30">
                                    ...
                                    <div className="flex gap-3">
                                        <button onClick={reset} className="size-12 rounded-2xl border border-slate-200 text-slate-500 flex items-center justify-center hover:bg-slate-50 transition-all" title="Làm mới"><span className="material-symbols-outlined text-2xl">refresh</span></button>
                                        <button onClick={() => window.print()} className="size-12 rounded-2xl border border-slate-200 text-slate-500 flex items-center justify-center hover:bg-slate-50 transition-all" title="Tải PDF"><span className="material-symbols-outlined text-2xl">download</span></button>
                                        <button 
                                            onClick={handleSaveLesson}
                                            className="bg-primary text-white px-8 py-3 rounded-2xl text-xs font-black hover:scale-105 transition-all shadow-lg shadow-primary/20 uppercase tracking-widest"
                                        >
                                            Lưu giáo trình
                                        </button>
                                    </div>
                                ...
                                <div className="p-10 flex-1 overflow-y-auto space-y-12">
                                    ...
                                    <div>
                                        <div className="flex items-center gap-3 mb-6">
                                            ...
                                            <h3 className="font-black text-xl text-slate-800 uppercase tracking-tight">Mục tiêu bài học</h3>
                                        </div>
                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                            {[
                                                ...
                                                <div key={i} className="p-6 bg-slate-50 rounded-3xl border border-slate-100 hover:shadow-md transition-all">
                                                    ...
                                                    <p className="text-sm font-bold text-slate-600 leading-relaxed">{goal.text}</p>
                                                </div>
                                            ))}
                                        </div>
                                    </div>
                                    ...
                                    <div>
                                        <div className="flex items-center gap-3 mb-6">
                                            ...
                                            <h3 className="font-black text-xl text-slate-800 uppercase tracking-tight">Hoạt động chính (STEAM)</h3>
                                        </div>
                                        <div className="space-y-6">
                                            {result.activities.map((act: any, i: number) => (
                                                <div key={i} className="flex gap-6 p-6 bg-white border border-slate-100 rounded-3xl hover:border-primary/30 transition-all group">
                                                    ...
                                                    <div>
                                                        <h4 className="font-black text-slate-800 mb-2 text-lg">{act.title}</h4>
                                                        <p className="text-sm font-medium text-slate-500 leading-relaxed">{act.desc}</p>
                                                    </div>
                                                </div>
                                            ))}
                                        </div>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                                        <div className="bg-pink-50/50 p-8 rounded-[32px] border border-pink-100 relative">
                                            <div className="flex items-center gap-3 mb-6">
                                                <div className="size-10 rounded-xl bg-pink-100 text-pink-600 flex items-center justify-center">
                                                    <span className="material-symbols-outlined text-2xl">auto_stories</span>
                                                </div>
                                                <h3 className="font-black text-xl text-slate-800 uppercase tracking-tight">Câu chuyện dẫn dắt</h3>
                                            </div>
                                            <p className="italic text-slate-700 leading-relaxed font-medium relative pr-8">
                                                <span className="material-symbols-outlined absolute -top-4 -right-2 text-pink-200 text-6xl rotate-180">format_quote</span>
                                                {result.story}
                                            </p>
                                        </div>
                                        ...
                                        <div className="bg-blue-50/50 p-8 rounded-[32px] border border-blue-100">
                                            <div className="flex items-center gap-3 mb-6">
                                                ...
                                                <h3 className="font-black text-xl text-slate-800 uppercase tracking-tight">Câu hỏi tương tác</h3>
                                            </div>
                                            <ul className="space-y-4">
                                                {result.questions.map((q: string, i: number) => (
                                                    <li key={i} className="flex items-start gap-4 text-sm font-black text-slate-600">
                ...
                <section>
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h2 className="text-2xl font-black text-slate-800 uppercase tracking-tight">Thư viện bài giảng của tôi</h2>
                                <p className="text-slate-400 text-xs font-bold mt-1 uppercase tracking-widest">Quản lý các giáo trình đã lưu</p>
                            </div>
                        </div>
                        <div className="flex gap-3">
                            <select className="bg-white border-none rounded-2xl text-[10px] font-black uppercase tracking-widest px-6 py-3 outline-none focus:ring-4 focus:ring-primary/10 shadow-sm text-slate-600">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
                        {[
                            ...
                            <div key={i} className="bg-white rounded-[40px] border border-slate-100 shadow-sm overflow-hidden group hover:shadow-2xl hover:-translate-y-2 transition-all duration-500">
                                ...
                                <div className="p-8">
                                    <h4 className="font-black text-slate-800 text-xl mb-6 group-hover:text-primary transition-colors leading-tight">{card.title}</h4>
                    ...
                    <div className="mt-12 text-center">
                        <button className="px-10 py-4 bg-slate-100 text-slate-500 rounded-2xl font-black text-xs uppercase tracking-[3px] hover:bg-primary hover:text-white transition-all shadow-sm">
