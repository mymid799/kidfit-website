...
interface User {
    ...
    staffProfile?: { full_name: string; phone: string | null; position: string; grade_block: string };
}
...
const AccountCreateForm = ({ onCancel, onSuccess, initialData }: { onCancel: () => void, onSuccess: () => void, initialData?: User | null }) => {
    ...
    const [formData, setFormData] = useState({
        ...
        grade_block: initialData?.staffProfile?.grade_block || 'ALL'
    });
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const payload: any = {
                ...
                role: formData.role,
                grade_block: formData.grade_block
            };
                ...
                <div className="p-8 md:p-12">
                    ...
                    <form className="space-y-8" onSubmit={handleSubmit}>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="relative">
                                    ...
                                    <select required value={formData.role} onChange={e => setFormData({ ...formData, role: e.target.value })}
                                        ...
                                        <option value="it_admin">Admin IT (Kỹ thuật)</option>
                                        <option value="principal">Hiệu trưởng</option>
                                        <option value="specialist">Tổ trưởng Chuyên môn</option>
                                        <option value="class_teacher">Giáo viên Chủ nhiệm</option>
                            ...
                            {['it_admin', 'principal', 'specialist', 'class_teacher', 'teacher'].includes(formData.role) && (
                                <div className="space-y-2">
                                    <label className="block text-sm font-bold text-on-surface/70 px-1">Khối phụ trách / Chuyên môn</label>
                                    <div className="relative">
                                        <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-slate-400">
                                            <span className="material-symbols-outlined">layers</span>
                                        </div>
                                        <select required value={formData.grade_block} onChange={e => setFormData({ ...formData, grade_block: e.target.value })}
                                            className="w-full pl-12 pr-10 py-4 bg-surface-container border-none rounded-full focus:ring-2 focus:ring-primary/20 focus:bg-white transition-all appearance-none text-on-surface text-sm font-bold cursor-pointer">
                                            <option value="ALL">Tất cả Khối (Mầm, Chồi, Lá)</option>
                                            <option value="MẦM">Chuyên trách khối MẦM</option>
                                            <option value="CHỒI">Chuyên trách khối CHỒI</option>
                                            <option value="LÁ">Chuyên trách khối LÁ</option>
                                        </select>
                                        <div className="absolute inset-y-0 right-0 pr-4 flex items-center pointer-events-none text-slate-400">
                                            <span className="material-symbols-outlined">expand_more</span>
                                        </div>
                        ...
                        <div className="pt-4 flex flex-wrap gap-3">
                            <span className="text-xs font-bold px-4 py-2 rounded-full bg-blue-100 text-blue-700 flex items-center gap-2">
                                <div className="w-1.5 h-1.5 rounded-full bg-blue-600"></div> IT Admin
                            </span>
                            <span className="text-xs font-bold px-4 py-2 rounded-full bg-red-100 text-red-700 flex items-center gap-2">
                                <div className="w-1.5 h-1.5 rounded-full bg-red-600"></div> Principal
                            </span>
                            <span className="text-xs font-bold px-4 py-2 rounded-full bg-amber-100 text-amber-700 flex items-center gap-2">
                                <div className="w-1.5 h-1.5 rounded-full bg-amber-600"></div> Specialist
                            </span>
                            <span className="text-xs font-bold px-4 py-2 rounded-full bg-emerald-100 text-emerald-700 flex items-center gap-2">
                                <div className="w-1.5 h-1.5 rounded-full bg-emerald-600"></div> Class Teacher
                            </span>
                        </div>
...
const UserManager = ({ initialRole = 'all' }: { initialRole?: string }) => {
    ...
    const ROLE_STYLES: Record<string, { bg: string, text: string, border: string, label: string, subtext: string }> = {
        it_admin: { bg: 'bg-blue-100', text: 'text-blue-700', border: 'border-blue-200', label: 'Admin IT', subtext: 'Quản trị hệ thống' },
        principal: { bg: 'bg-red-100', text: 'text-red-700', border: 'border-red-200', label: 'Principal', subtext: 'Ban Giám Hiệu' },
        specialist: { bg: 'bg-amber-100', text: 'text-amber-700', border: 'border-amber-200', label: 'Specialist', subtext: 'Tổ trưởng Chuyên môn' },
        class_teacher: { bg: 'bg-emerald-100', text: 'text-emerald-700', border: 'border-emerald-200', label: 'Teacher', subtext: 'Giáo viên Chủ nhiệm' },
        ...
        admin: { bg: 'bg-purple-100', text: 'text-purple-700', border: 'border-purple-200', label: 'Admin', subtext: 'Quản trị viên cũ' },
        teacher: { bg: 'bg-secondary-container/20', text: 'text-secondary', border: 'border-secondary/20', label: 'Teacher', subtext: 'Giáo viên cũ' },
    };
                ...
                <div className="flex flex-wrap items-center gap-4">
                    <div className="flex items-center gap-2">
                        ...
                        >
                            ...
                            <option value="principal">Hiệu trưởng</option>
                            <option value="specialist">Tổ trưởng</option>
                            <option value="class_teacher">GV Chủ nhiệm</option>
                            <option value="parent">Phụ huynh</option>
                            <option value="student">Học sinh</option>
                        </select>
                    </div>
...
const RoleAssignment = () => {
    ...
    const hasPermission = (role: any, mod: string, act: string) => {
        return role.permissions?.some((p: any) => p.module_name === mod && p.action === act);
    };

    const togglePermission = async (roleId: number, mod: string, act: string) => {
        const perm = permissions.find(p => p.module_name === mod && p.action === act);
        if (!perm) return;

        const role = roles.find(r => r.id === roleId);
        if (!role) return;

        let newPermIds = role.permissions.map((p: any) => p.id);
        if (newPermIds.includes(perm.id)) {
            newPermIds = newPermIds.filter((id: number) => id !== perm.id);
        } else {
    ...
    if (loading) return <div className="p-10 text-center font-bold text-slate-400 animate-pulse text-sm uppercase tracking-widest">Đang tải ma trận quyền hạn...</div>;

    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h2 className="text-2xl font-black text-slate-800 tracking-tight">Vai trò & Phân quyền</h2>
                    <p className="text-sm text-slate-500">Phân quyền chi tiết theo từng module chức năng cho các nhóm người dùng.</p>
                </div>
                <button className="bg-slate-900 text-white px-5 py-2.5 rounded-2xl font-bold text-sm shadow-sm hover:shadow-md transition-all flex items-center gap-2">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {roles.map(role => (
                    <div key={role.id} className="bg-white rounded-[32px] border border-slate-100 shadow-sm overflow-hidden flex flex-col">
                            ...
                            <div className="flex items-center gap-4">
                                <div className={`w-12 h-12 rounded-2xl bg-white shadow-sm flex items-center justify-center text-blue-500`}>
                                    <span className="material-symbols-outlined text-[28px]" style={{ fontVariationSettings: "'FILL' 1" }}>
                                        {role.name === 'parent' ? 'groups' : (role.name === 'it_admin' ? 'shield' : 'school')}
                                    </span>
                                </div>
                                <div>
                                    <h3 className="font-black text-slate-800 tracking-tight uppercase text-sm">{role.display_name}</h3>
                                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{role.name}</p>
                                </div>
                            </div>
                            ...
                            <table className="w-full">
                                <thead>
                                    <tr className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
                                        ...
                                        {actions.map(a => <th key={a} className="px-2 py-2 text-center w-8">{a}</th>)}
                                    </tr>
                                </thead>
                                    ...
                                    {modules.map(mod => (
                                        <tr key={mod} className="hover:bg-slate-50/50 transition-colors">
                                            <td className="px-4 py-3">
                                                <p className="text-[13px] font-bold text-slate-700 capitalize">{mod}</p>
                                            </td>
                                            {actions.map(action => (
                                                <td key={action} className="px-2 py-3 text-center">
                                                    <label className="relative inline-flex items-center cursor-pointer group">
                                                        <input type="checkbox" className="sr-only peer" 
                                                            checked={hasPermission(role, mod, action)}
                                                            onChange={() => togglePermission(role.id, mod, action)} 
                                                        />
                                                        <div className="w-6 h-6 bg-slate-100 peer-checked:bg-kids-blue rounded-lg transition-all flex items-center justify-center">
                        ...
                        <div className="px-6 py-4 bg-slate-50/30 border-t border-slate-100 flex justify-end gap-3 font-bold text-xs uppercase tracking-wider">
                            <button className="text-slate-400 hover:text-slate-600">Hủy</button>
                            <button className="text-kids-blue">Lưu thay đổi</button>
                        </div>
                    </div>
                ))}
            </div>
...
// ============================================================
// LESSON MANAGEMENT COMPONENT (WITH DIGITAL SIGNING)
// ============================================================
const LessonManagement = () => {
    ...
    const handleSign = async (id: number) => {
        const pin = prompt('Nhập mã PIN ký số (Mặc định: 1234):');
        if (!pin) return;

        try {
            const res = await fetch(`${API}/teacher/materials/lessons/${id}/sign`, {
                method: 'POST',
                headers: getHeaders(),
            ...
            const data = await res.json();
            if (data.success) {
                alert('Đã ký duyệt giáo án thành công!');
                fetchLessons();
            } else {
                alert(data.error || 'Lỗi ký duyệt');
            }
        } catch (error) {
            alert('Lỗi hệ thống khi ký duyệt');
        }
    };
    ...
    const getStatusColor = (status: string) => {
        switch (status) {
            ...
            case 'VERIFIED': return 'bg-purple-100 text-purple-600 border-purple-200';
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h2 className="text-2xl font-black text-slate-800 tracking-tight uppercase">Quản lý Giáo án & Ký duyệt</h2>
                    <p className="text-sm font-bold text-slate-400 uppercase tracking-widest mt-1">Hệ thống lưu trữ & phê duyệt số chuẩn ISO</p>
                </div>
                <div className="flex gap-2 p-1.5 bg-slate-100 rounded-2xl w-fit">
                    {['ALL', 'SUBMITTED', 'VERIFIED', 'APPROVED'].map(f => (
                        <button 
                            key={f}
                            onClick={() => setFilter(f)}
                            className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-wider transition-all ${filter === f ? 'bg-white text-kids-blue shadow-sm' : 'text-slate-400 hover:text-slate-600'}`}
                        >
                            {f === 'ALL' ? 'Tất cả' : f}
                        </button>
                    ))}
                ...
                <table className="w-full text-left">
                    <thead>
                        <tr className="bg-slate-50/50 text-[10px] font-black uppercase tracking-[2px] text-slate-400">
                            <th className="px-8 py-5">Giáo án / Chủ đề</th>
                            <th className="px-6 py-5">Người soạn</th>
                            <th className="px-6 py-5">Trạng thái</th>
                            <th className="px-6 py-5">Ký số (Signature)</th>
                            <th className="px-8 py-5 text-right">Thao tác</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-slate-50">
                        {loading ? (
                            <tr><td colSpan={5} className="py-20 text-center font-bold text-slate-400 animate-pulse uppercase tracking-[4px]">Đang tải kho lưu trữ...</td></tr>
                            ...
                            <tr key={lesson.id} className="hover:bg-slate-50/50 transition-colors group">
                                <td className="px-8 py-5">
                                    <div className="flex items-center gap-4">
                                        <div className="w-12 h-12 bg-slate-100 rounded-2xl flex items-center justify-center text-slate-400 group-hover:bg-kids-blue/10 group-hover:text-kids-blue transition-colors">
                                            <span className="material-symbols-outlined text-[28px]">description</span>
                                        </div>
                                        <div>
                                            <p className="font-black text-slate-800 text-sm tracking-tight">{lesson.title}</p>
                                            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">{lesson.grade_block}</p>
                                        </div>
                                    </div>
                                </td>
                                <td className="px-6 py-5">
                                    <p className="text-sm font-bold text-slate-600">NV0{lesson.creator_id || 1}</p>
                                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Giáo viên</p>
                                </td>
                                <td className="px-6 py-5">
                                    <span className={`px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-wider border ${getStatusColor(lesson.status)}`}>
                                        {lesson.status}
                                    </span>
                                </td>
                                <td className="px-6 py-5">
                                    {lesson.digital_signature ? (
                                        <div className="flex flex-col">
                                            <div className="flex items-center gap-2 text-emerald-600">
                                                <span className="material-symbols-outlined text-[18px]">verified_user</span>
                                                <span className="text-[11px] font-black tracking-tighter">AUTHENTICATED</span>
                                            </div>
                                            <p className="text-[8px] font-mono text-slate-400 truncate w-32">{lesson.digital_signature}</p>
                                        </div>
                                    ) : (
                                        <span className="text-[10px] font-bold text-slate-300 italic uppercase">Chưa ký số</span>
                                    )}
                                </td>
                                <td className="px-8 py-5 text-right">
                                    <div className="flex justify-end gap-2">
                                        <button className="p-2.5 bg-slate-50 hover:bg-slate-100 text-slate-500 rounded-xl transition-all active:scale-95">
                                            <span className="material-symbols-outlined text-[20px]">visibility</span>
                                        </button>
                                        {lesson.status === 'VERIFIED' && (
                                            <button 
                                                onClick={() => handleSign(lesson.id)}
                                                className="px-4 py-2 bg-kids-blue hover:bg-blue-600 text-white rounded-xl text-[10px] font-black uppercase tracking-wider shadow-md shadow-blue-200 transition-all active:scale-95 flex items-center gap-2"
                                            >
...
const AdminDashboard = () => {
    ...
    const navItems = [
        ...
        {
            id: 'lessons', label: 'Giáo án STEAM',
        ...
        {
            id: 'role-permissions', label: 'Vai trò & Quyền',
            path: 'M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04M12 2.944a11.955 11.955 0 01-8.618 3.04m0 0a11.955 11.955 0 00-1.764 7.802 11.955 11.955 0 005.103 8.307m-3.339-16.11a11.955 11.955 0 018.618-3.04m0 0a11.955 11.955 0 018.618 3.04m0 0a11.955 11.955 0 001.764 7.802 11.955 11.955 0 00-5.103 8.307M12 21.056a11.955 11.955 0 01-8.618-3.04m0 0l-5.103-8.307a11.955 11.955 0 011.764-7.802'
        },
        {
        ...
        <div className="flex h-screen overflow-hidden" style={{ fontFamily: "'Quicksand', sans-serif", backgroundColor: '#F8FAFC', color: '#334155' }}>
            ...
            <main className="flex-1 flex flex-col min-w-0 overflow-hidden w-full">
                ...
                <div className="flex-1 overflow-y-auto p-4 md:p-6" style={{ scrollbarWidth: 'thin' }}>
                    ...
                    {activeTab === 'role-permissions' && <RoleAssignment />}
                    {activeTab === 'lessons' && <LessonManagement />}
                    {!['overview', 'accounts', 'classes', 'attendance', 'staff', 'settings', 'lessons', 'role-permissions'].includes(activeTab) && (
